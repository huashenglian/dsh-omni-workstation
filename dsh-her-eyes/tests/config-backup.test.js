...
const cfgDir = mkdtempSync(join(tmpdir(), 'her-eyes-cfg-'))
const bakDir = mkdtempSync(join(tmpdir(), 'her-eyes-bak-'))
process.env.DSH_HER_EYES_CONFIG_DIR = cfgDir
...
test('storeConfig writes both main and backup files', async () => {
  ...
  const mainFile = join(cfgDir, 'vlm-vision.json')
