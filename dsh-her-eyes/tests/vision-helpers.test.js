...
test('saveArtifact 落盘到 exec.meta.cwd/.her-eyes/artifacts', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'her-eyes-art-'))
  const exec = { agent: { meta: { cwd } } }
  const file = saveArtifact(exec, 'x_123.png', Buffer.from([1, 2, 3]))
  assert.ok(existsSync(file))
  assert.equal(file, join(cwd, '.her-eyes', 'artifacts', 'x_123.png'))
})