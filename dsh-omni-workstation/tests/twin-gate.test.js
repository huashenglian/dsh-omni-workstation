// twin-gate.test.js — v1.8: gate wiring + adapters-updated + agent/request tracking + /omni/config twinVisible
...
const dshHome = mkdtempSync(join(tmpdir(), 'omni-workstation-gate-'))
...
test('adapters-updated handler reconciles twin from stored config', async () => {
  await resetTwins()
  writeFileSync(join(dshHome, 'omni-vision.json'), validConfig(9))
  const ctx = makeFakeCtx(['alpha'])
  apply(ctx)
...
test('/omni/config GET reports twinVisible with the gate', async () => {
  await resetTwins()
  writeFileSync(join(dshHome, 'omni-vision.json'), validConfig(9))
  const ctx = makeFakeCtx(['alpha'])
  apply(ctx)
  const route = ctx.routes.find((r) => r.path === '/omni/config')
  assert.ok(route, 'apply registers the /omni/config route')
  let payload = null
  const res = {
    writeHead: () => {},
...
test('/omni/config GET reports twinVisible=false when VLM disabled', async () => {
  await resetTwins()
  writeFileSync(join(dshHome, 'omni-vision.json'), JSON.stringify({
    vlmEnabled: false,
...
test('v1.8: agent/request tracks lastSourceProvider; twin route stays auto-vision (no re-register)', async () => {
  ...
  // 'beta-omni-workstation'. The stream reads lastSourceProvider at call time.
