...
async function main() {
  ...
  if (!initial || !initial.ok) {
    console.log('FATAL: dsh web not running or /omni/config failed')
    process.exit(1)
  }
  ...
  // 9. Test /omni/all-models endpoint
  console.log('\n--- Step 9: Test /omni/all-models endpoint ---')
  try {
    const amRes = await fetch(BASE + '/omni/all-models', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{}'
    })
    ...
    if (amJson.ok) {
      console.log('  /omni/all-models returned ' + (amJson.groups?.length || 0) + ' provider groups')
      ok(Array.isArray(amJson.groups), 'all-models groups is array')
    } else {
      console.log('  /omni/all-models error: ' + (amJson.error || 'unknown'))
      ok(false, '/omni/all-models should return ok')
    }
  } catch (e) {
    ok(false, '/omni/all-models endpoint not reachable: ' + e.message)
  }
