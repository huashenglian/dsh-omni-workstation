...
const dshHome = mkdtempSync(join(tmpdir(), 'omni-workstation-vtools-'))
...
const writeCfg = (obj) => writeFileSync(join(dshHome, 'omni-vision.json'), JSON.stringify(obj))
...
test('normalizeConfig defaults visionToolsEnabled to true (backward compatible)', async () => {
  ...
  // The settings page reads masked(cfg).visionToolsEnabled via GET /omni/config;
...
test('applyPatch can toggle visionToolsEnabled (via /omni/config POST)', async () => {
  _resetVisionTools()
  writeCfg({})
  const ctx = makeFakeCtx()
  apply(ctx)
