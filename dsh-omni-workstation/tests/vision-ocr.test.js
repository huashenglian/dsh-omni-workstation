...
test('ocr_image engine=local 探测不可用 → LOCAL_OCR_UNAVAILABLE', async () => {
  // 把 OMNI_WORKSTATION_TESSERACT 指向 node 可执行文件：--version 探测成功（缓存为 node），
  // 但真正以 tesseract 参数调用会失败 → 结果确定地返回 LOCAL_OCR_UNAVAILABLE，
  // 与系统是否安装 Tesseract 无关。
  process.env.OMNI_WORKSTATION_TESSERACT = process.execPath
  ...
  try {
    ...
    delete process.env.OMNI_WORKSTATION_TESSERACT
    _resetTesseractProbe()
  }
})
