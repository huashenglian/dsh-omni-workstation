...
process.env.DSH_HOME = mkdtempSync(join(tmpdir(), 'omni-workstation-mirror-'))
...
test('mirrorRouteId prefixes with omni-workstation-m-', () => {
  assert.equal(mirrorRouteId('my-model'), 'omni-workstation-m-my-model')
  assert.equal(mirrorRouteId(''), 'omni-workstation-m-mirror')
})
...
test('Mode 2: mirrorAllEnabled=true → per-provider <provider>-omni-workstation twins', async () => {
  await resetTwins()
  ...
  assert.ok(ctx.registrations.has('alpha-omni-workstation'))
  assert.ok(ctx.registrations.has('beta-omni-workstation'))
  assert.equal(ctx.registrations.has('auto-vision'), false)
})

test('Mode 2: excludes -omni-workstation, -vision, omni-workstation-m- routes from per-provider twins', async () => {
  await resetTwins()
  const ctx = makeFakeCtx(['gamma', 'gamma-omni-workstation', 'gamma-vision', 'omni-workstation-m-foo'])
  await syncTwins(ctx, { mirrorConfig: { autoVisionEnabled: false, mirrorAllEnabled: true, mappings: [] } }, true)
  assert.ok(ctx.registrations.has('gamma-omni-workstation'))
  assert.equal(ctx.registrations.size, 1, 'only gamma-omni-workstation should be registered')
})

test('Mode 3: custom mappings → omni-workstation-m-* twins', async () => {
  await resetTwins()
  const ctx = makeFakeCtx(['alpha'])
  await syncTwins(ctx, {
    mirrorConfig: {
  ...
  assert.ok(ctx.registrations.has('omni-workstation-m-my-vision'))
  assert.equal(ctx.registrations.has('auto-vision'), false)
})
...
test('Mode 3: mapping with empty originalModel is kept in config but NOT registered as twin', async () => {
  ...
  assert.ok(ctx.registrations.has('omni-workstation-m-ok'))
  assert.equal(ctx.registrations.has('omni-workstation-m-pending'), false, 'incomplete mapping should not register a twin')
})
...
test('Mode 3: empty mirrorName → route uses <originalModel>-vision', async () => {
  ...
  assert.ok(ctx.registrations.has('omni-workstation-m-model-a-vision'))
})
...
test('Mode 2 masks Mode 3: mirrorAllEnabled=true + mappings → only per-provider twins, no mapping twins', async () => {
  ...
  assert.ok(ctx.registrations.has('alpha-omni-workstation'))
  assert.equal(ctx.registrations.has('omni-workstation-m-custom'), false, 'mapping twin should be masked by mirrorAll')
})
...
test('syncTwins disposes twins when config changes (mirrorAll off → per-provider twins disposed)', async () => {
  ...
  assert.ok(ctx.registrations.has('alpha-omni-workstation'))
  await syncTwins(ctx, { mirrorConfig: { autoVisionEnabled: false, mirrorAllEnabled: false, mappings: [] } }, true)
  assert.equal(ctx.registrations.has('alpha-omni-workstation'), false)
  assert.ok(ctx.disposed.includes('alpha-omni-workstation'))
})
...
test('syncTwins disposes mapping twins when mapping removed', async () => {
  ...
  assert.ok(ctx.registrations.has('omni-workstation-m-custom'))
  const cfg2 = { mirrorConfig: { autoVisionEnabled: false, mirrorAllEnabled: false, mappings: [] } }
  await syncTwins(ctx, cfg2, true)
  assert.equal(ctx.registrations.has('omni-workstation-m-custom'), false)
})
...
test('makePerProviderTwinAdapter: providerInfo/listModels/resolveModel', async () => {
  ...
  assert.equal(info.id, 'alpha-omni-workstation')
  assert.ok(info.name.includes('Auto Vision'))
  const models = await adapter.listModels()
  assert.equal(models.length, 2)
  assert.deepEqual(models[0].inputModalities, ['text', 'image'])
  assert.equal(models[0].provider, 'alpha-omni-workstation')
})
...
test('makeMappingTwinAdapter: single model with mirror name', async () => {
  ...
  assert.deepEqual(models[0].inputModalities, ['text', 'image'])
  const resolved = await adapter.resolveModel('omni-workstation-m-my-vision', 'my-vision')
