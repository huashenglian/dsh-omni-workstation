...
".omni-settings-section-title { font-size: 13px; font-weight: 600; opacity: 0.8; }",
  ...
  ".omni-subtab-btn.active { background: #2a2a2e; border-color: #4a9eff; color: #fff; }",
	...
	React.createElement("button", {
		...
		title: t("presetSaved"),
		disabled: !igDirty,
		onClick: function () { if (presetNameDraft[0] !== (props.activePresetName || "")) props.onRenamePreset(presetNameDraft[0]); props.onSavePreset(); igSnapshot[1](JSON.stringify({ name: presetNameDraft[0], cfg: props.cfg || {} })); }
