...
voiceSovitsModel: "SoVITS 模型名",
			...
			verifyReminderLabel: "生图后自动验证提醒（需 VLM 开启）",
...
voiceSovitsModel: "SoVITS model name",
			...
			verifyReminderLabel: "Auto-verify reminder after image generation (requires VLM on)",
  ...
  ".omni-subtab-btn.active { background: #2a2a2e; border-color: #4a9eff; color: #fff; }",
	...
	var I_TOAST_ERROR = ["M8 2a6 6 0 100 12A6 6 0 008 2zm3 3L5 11M11 5L5 11"];
		...
		function ApiCard(props) {
			...
			return React.createElement("div", {
				className: "omni-card" + (isDragging ? " dragging" : "") + (isDropTarget ? " drop-target" : ""),
				onDragOver: dragOver,
				onDrop: drop,
				onDragEnd: dragEnd
			}, [head, body]);
...
return React.createElement("div", { className: "omni-imggen-panel" }, [head, presetBar, React.createElement("div", { className: "omni-preset-divider" }), body, confirmModal]);
				...
				var presetBar = React.createElement("div", { className: "omni-preset-bar" }, [
					React.createElement("div", { className: "omni-preset-input-wrap" }, [
						React.createElement("input", {
							...
							placeholder: t("presetNamePh"),
							onChange: function (e) { presetNameDraft[1](e.target.value); },
							onBlur: function (e) { if (e.target.value !== activePresetName) props.onRenamePreset(e.target.value, activeSubtab[0]); }
						}),
						React.createElement("button", {
							className: "omni-preset-dd-btn", type: "button",
							onClick: props.onTogglePresetDd
				...
				var ttsBody = React.createElement("div", { className: "omni-card-body" + (props.ttsEnabled ? "" : " omni-tab-disabled") }, [
					...
					React.createElement("div", { className: "omni-row" }, [
						...
						React.createElement("div", { className: "omni-module-row" }, [
							...
							React.createElement("p", { className: "omni-voice-hint" }, t("voiceFilterTtsDesc")),
					...
					React.createElement("p", { className: "omni-status" },
						(function () {
							...
							// v2.9.4: only mimo/minimax have synthesis wired; other providers register no voice tools
							if (cfg.provider !== "mimo" && cfg.provider !== "minimax" && cfg.provider !== "doubao") return t("voiceStatusNoSynth");
	...
	function HelpModal(props) {
		...
		var igPresetNameDraft = React.useState("");
			...
			var videoModelCount = React.useState(null);
			var videoPresetDdOpen = React.useState(false);
			var videoPresetDeleteConfirm = React.useState(false);
		...
		React.useEffect(function () {
			...
			React.useEffect(function () {
				if (!menuOpen[0] && !openDd[0] && !batchOpen[0] && !openProv[0] && !igOpenDd[0] && !igOpenProv[0] && !igConfirmReset[0] && !fbOpenDd[0] && !mirrorOpenDd[0] && !videoOpenDd[0] && !videoOpenProv[0] && !voiceOpenDd[0] && !voiceOpenProv[0] && !voicePresetDdOpen[0] && !voiceRefDdOpen[0]) return;
				function close(e) {
					var tgt = e.target;
					if (!tgt || !tgt.closest) {
						...
						videoOpenDd[1](false); videoOpenProv[1](false); videoPresetDdOpen[1](false);
						voiceOpenDd[1](false); voiceOpenProv[1](false); voicePresetDdOpen[1](false); voiceRefDdOpen[1](false);
						confirmDel[1](null); confirmDelAll[1](false); return;
					}
					...
					if (!tgt.closest(".omni-preset-bar")) { videoPresetDdOpen[1](false); voicePresetDdOpen[1](false); voiceRefDdOpen[1](false); }
				}
				document.addEventListener("click", close);
				return function () { document.removeEventListener("click", close); };
			}, [menuOpen[0], openDd[0], batchOpen[0], openProv[0], igOpenDd[0], igOpenProv[0], igConfirmReset[0], fbOpenDd[0], mirrorOpenDd[0], videoOpenDd[0], videoOpenProv[0], voiceOpenDd[0], voiceOpenProv[0], voicePresetDdOpen[0], voiceRefDdOpen[0]]);
		...
		function commitStructure(patch, localFn, onSuccess) {
			...
			function fetchVideoModels() {
				...
				call("models", {
					...
					if (r && r.ok && Array.isArray(r.models) && r.models.length > 0) {
						...
						videoModelCount[1](r.models.length);
						videoOpenDd[1](true);
						showToast('success', t('fetchOkPrefix') + r.models.length + t('fetchOkSuffix'), '');
					} else {
			...
			// ---- video preset management (v2.8, mirrors imggen presets) ----
			function switchVideoPreset(id) {
				videoPresetDdOpen[1](false);
				videoKeyDraft[1]("");
				videoRevealed[1](false);
				videoModelList[1]([]);
				videoModelCount[1](null);
				commitStructure({ videoPresetSwitch: id }, null, function () { showToast('success', t('presetSwitched'), ''); });
			}
			function addVideoPreset() {
				...
				videoModelList[1]([]);
				videoModelCount[1](null);
				commitStructure({ videoPresetAdd: true }, null, function () { showToast('success', t('presetAdded'), ''); });
			}
			function deleteVideoPreset(id) {
				videoPresetDeleteConfirm[1](false);
				videoKeyDraft[1]("");
				videoRevealed[1](false);
				videoModelList[1]([]);
				videoModelCount[1](null);
		...
		function onDoubaoClonePresetPatch(fields) {
			...
			updateDraft(function (d) { var dcp = (d.doubaoClonePresets || []).find(function (p) { return p.id === d.activeDoubaoClonePreset; }); if (dcp) { if (typeof fields.speakerId === 'string') dcp.speakerId = fields.speakerId; if (typeof fields.refAudioPath === 'string') dcp.refAudioPath = fields.refAudioPath; } return d; });
		}
			...
			function fetchImggenModels() {
				...
				if (r && r.ok) {
					...
					igVaeList[1](r.vae || []);
					igClipList[1](r.clip || []);
					igModelCount[1](fetchedModels.length);
					if (fetchedModels.length > 0) igOpenDd[1](true); // auto-open dropdown
					showToast('success', t('fetchOkPrefix') + fetchedModels.length + t('fetchOkSuffix'), '');
				} else {
						setMsg(t("fetchFail") + (r && r.error ? r.error : t("unknown")), true);
						showToast('error', t("fetchFail"), (r && r.error) || t("unknown"));
					}
		...
		function togglePresetDd() {
			...
			var videoPresets = draft[0].videoPresets || [];
			var activeVideoPresetId = draft[0].activeVideoPreset || (videoPresets.length > 0 ? videoPresets[0].id : "");
			var activeVideoPresetName = (videoPresets.find(function (p) { return p.id === activeVideoPresetId; }) || {}).name || "";
				...
				React.createElement("div", { style: { display: "flex", gap: "8px", alignItems: "center" } }, [
					...
					React.createElement("div", { className: "omni-batch-wrap" }, [
						...
						batchOpen[0] ? React.createElement("div", { className: "omni-batch-menu", ref: menuDdRef }, [
							...
							React.createElement("div", { className: "omni-menu-item omni-menu-danger" + (confirmDelAll[0] ? " omni-menu-confirm" : ""), onClick: batchDeleteAll },
			...
			var imggenBody = React.createElement("div", { className: "omni-tab-body" }, [
				...
				var videoBody = React.createElement("div", { className: "omni-tab-body" }, [
					React.createElement(VideoPanel, {
						...
						modelCount: videoModelCount[0],
						presets: videoPresets,
						activePresetId: activeVideoPresetId,
						activePresetName: activeVideoPresetName,
						presetDdOpen: videoPresetDdOpen[0],
						presetDeleteConfirm: videoPresetDeleteConfirm[0],
						onSwitchPreset: switchVideoPreset,
						onAddPreset: addVideoPreset,
						onDeletePreset: deleteVideoPreset,
						onRenamePreset: renameVideoPreset,
						onTogglePresetDd: toggleVideoPresetDd,
						onConfirmDeletePreset: function () { videoPresetDeleteConfirm[1](true); },
						onCancelDeletePreset: function () { videoPresetDeleteConfirm[1](false); },
						onConfirmDelete: function () { deleteVideoPreset(activeVideoPresetId || ""); },
						onPatch: patchVideo,
						onSaveKey: saveVideoKey,
						onToggleReveal: toggleVideoReveal,
						onFetchModels: fetchVideoModels,
						onPickModel: pickVideoModel,
						onToggleDd: function () { var n = !videoOpenDd[0]; videoOpenDd[1](n); if (n && videoModelList[0].length === 0) fetchVideoModels(); },
