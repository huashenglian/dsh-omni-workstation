// dsh-omni-workstation — 视觉工具箱宿主模块（vision toolkit host-side tool defs）
//
...
function probeTesseract() {
  ...
  const candidates = []
  if (process.env.OMNI_WORKSTATION_TESSERACT) candidates.push(process.env.OMNI_WORKSTATION_TESSERACT)
...
export function buildVisionToolDefs(deps) {
  ...
  const showImageTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      let path
      if (attachmentId) {
        // 附件需先物化到 images 目录（本地路径则直接用解析后的绝对路径）
