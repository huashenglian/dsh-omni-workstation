...
const VIDEO_PROVIDERS = {
  ...
  // 通义千问（Qwen）Token Plan：OpenAI 兼容网关（token-plan.*.maas.aliyuncs.com/compatible-mode/v1），
  // 视频走 Sora 风格 openai-videos 协议（POST {base}/videos）。端点/协议锁定，仅填 Key + 模型。
  'qwen-token-plan':   { protocol: 'openai-videos', endpoint: 'https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1', keyRequired: true, fixedUrl: true, fixedProtocol: true },
  'qwen-token-plan-cn':{ protocol: 'openai-videos', endpoint: 'https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1', keyRequired: true, fixedUrl: true, fixedProtocol: true }
}
